...
int main (int argc, char *argv[]) {
	...
	for(i = transf_hierarchy[box_index][2] ; i >= 0 ; i = transf_hierarchy[i][0]) {
		if(contourArea(transf_contours[i],false) > 100) {
			...
			if (area < AREA_THRESH_MIN || area > AREA_THRESH_MAX) continue;
			avg_rows += sqrt(area);
			...
			int index = FindIndex( mid_pt, img.cols, img.rows );
			 t++;
			i1 = 0;
//			if (box_vertices.size() != 4) cout << index << " " << box_vertices.size() << endl;
	...
	for(i = 0 ; i < 9 ; i++) {
		for(j = 0 ; j < 9 ; j++) {
			...
			////Mat dummy5 = box[i*9+j].clone();
			////double arr = can_be_no2 ( dummy5 );
